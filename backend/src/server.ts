...
async function seedDatabaseIfEmpty() {
  try {
    ...
    console.log('Database empty. Running auto-seeding routine...');
    ...
    const provider1 = savedProviders[0]; // Ravi
    const provider2 = savedProviders[1]; // Anjali
...
async function startServer() {
  ...
  // Auto-seed if running on empty / fresh instance
  await seedDatabaseIfEmpty();

  const PORT = parseInt(envs.PORT, 10);
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`Server is running on port ${PORT} in ${envs.NODE_ENV} mode`);
  });
}
